...
En el proceso de extracción, se emplea el paquete `rvest` para analizar el contenido de la página web y el paquete `dplyr` para combinar la lista de tablas en el dataframe, adicionalmente, se aplica una primera limpieza de los datos con el condicional `if` para eliminar la columna innecesaria de índice.
...
Así mismo, para poder trabajar con los datos de forma adecuada, es necesario realizar un análisis descriptivo de las variables seleccionadas como se muestra a continuación:
...
-   **Estadísticas descriptivas por variables:**
    ...
    -   *age:* la variable no cuenta con valores faltntes, indicando el 100% de sus valores completos. La edad promedio de los individuos es de 39 años, con una desviación estándar de 13,48. La edad mínima registrada es 18 años, el primer cuartil corresponde a 28 años, la mediana a 38 años, el tercer cuatil a 50 años y el valor máximo es de 94 años. A partir del histograma, se observa unaóconcentración promedio en edades menores al segundo y tercer cuaril.
    ...
    -   *max_nivel_educ:* la variable categorica para el nivel educativo presenta 1 valor faltante, por lo tanto, tiene un 99,99% de los datos completos. Ahora para comprender el análisis de las estadísticas es necesario conocer las respectivas categorías del máximo nivel educativo alcanzado o el último grado aprovado:
        ...
        Analizando el histograma, se puede concluir que la mayor frecuencia de respuesta de las personas respecto a su nivel educativo es "No sabe, no informa".

    -   *tiempo_empresa:* no tiene valores faltantes y tiene un 100% de los datos completos. El tiempo de trabajo en la empresa se encuentra medido en meses, donde el promedio es de 63,76 meses (5,3 años) con una desviación estándar de 89,48. El valor mínimo (primer cuartil) es de 0 meses, indicando probablemente que las personas en el momento de la encuesta habían iniciado a trabajar y no habían cumplido un mes en la empresa. La mediana es 24 meses (2 años) y el tercer cuartil corresponde a 84 meses (7 años). Finalmente, el máximo tiempo registrado del tiempo de trabajo en la empresa es de 720 meses (60 años), es importante resaltar que de acuerdo a las normativas laborales colombianas, una persona con trabajo formal puede llegar a trabajar en promedio entre 35 a 40 años, sin emabargo, es posible que esto no aplique para personas con trabajos informales. Por último, se observa que el histograma tiene una cola a la derecha, indicando que la gran parte de la muestra se encuentra distribuida entre el primer y segundo cuartil.

    -   *tamaño_empresa:* no tiene valores faltantes y tiene un 100% de los datos completos. Ahora para comprender el análisis de las estadísticas es necesario conocer las respectivas categorías del tamaño de la empresa:
    ...
    -   *depto:* no tiene valores faltantes y tiene un 100% de los datos completos. Esta variable contiene el código del departamento (11), el cuál se ha identificado como único valor en df_salario, haciendo referencia que todas las observaciones del data frame son personas que viven en Bogotá.

    -   *formal:* no tiene valores faltantes y tiene un 100% de los datos completos. Dado que es una variable dicotómica, no se considera relevante explicar otras estadísticas. Sin embargo, toma el valor de 1 cuando la persona tiene un trabajo formal y 0 cuando la persona tiene un trabajo informal.
    ...
    -   *segu_social:* la variable tiene 1.403 valores faltante, es decir que cuenta con un 91,51% de datos completos, como es una variable categórica, es necesario conocer las mismas para comprender el análisis de las estadísticas:
        ...
        A partir del histograma, la distribución de la muestra de acuerdo a las categorías se centra principalmente en la categoría 1, sin embargo, también se debe considerar a la categoría 2.
...
# Guardar  
ggsave("GRÁFICO1-VALORES FALTANTES.png", dpi = 300)
...
# Guardar 
ggsave("GRÁFICO2-SALARIO HORA.png", dpi = 300)
...
# Tabla modelo1
t_modelo1 <- stargazer(modelo_age_wage, 
                       type = "text",
...
# Guardar tabla
writeLines(t_modelo1, "TABLA 1-MODELO DE SALARIO.txt")
```

### Interpretación de los resultados de la regresión log (salario)

El resultado de la estimación indica que el coeficiente $Age$ es positivo y $Age^2$ es negativo, esto predetermina que, valores pequeños en $Age$ tiene un efecto positivo en $log(w)$, por el contrario valores grandes en $Age$ genera una condicion de decrecimiento sobre el $log(w)$. De esta manera, se toma los multiplos de 18 años para realizar el análisis del cambio $\Delta log(\hat{w})$. 

Se observa que cuando la variable $Age$ es igual a 18 años, $\Delta log(\hat{w})$ es igual a 2,66% Ahora, se cambia el valor a 36 años, $\Delta log(\hat{w})$ es igual a 0,69%. Por último, $Age$ será 54 años y por lo tanto $\Delta log(\hat{w})$ es igual a -1,29%, lo cual responde a lo mencionado al principio de esta sección.
...
# Guardar
ggsave("GRÁFICO 3-PERFIL INGRESOS EDAD.png", dpi = 300)
...
*Bondad de ajuste:* El R² es 0.0194 y R² ajustado da el valor de 0.01932 , esto indica que el modelo explica el salario en un 1.93%. Estos valores bajos indican que el modelo captura una parte de la relación y existen otros factores que afectan el logaritmo de salario no incluidos en el modelo.

### Análisis de la “edad pico” con sus respectivos intervalos de confianza.

Luego de estimar, analizar las ecuaciones y las gráficas se puede sugerir que el comportamiento del salario en función de la edad en la población encuestada indica que los salarios son bajos a la edad de 18 años. Además, se notó que el salario mejora con el aumento de los años. No obstante, el ritmo de crecimiento del salario es más grande en comparación con edades medias, como por ejemplo, los 36 años, donde se registro un cambio de 0,69%. De otro lado, se registró la edad pico para este estudio en 42.25 años de edad, este punto es importante por dos razones. La primera, es el punto de inflexión entre el cambio positivo y el cambio negativo en el ingreso. La segunda, que en promedio se referencia la edad pico a los 50 años, esto puede sugerir que pueden haber otras variables que no fueron incluidas en la estimación que generen una disminución de la edad y así obtener mejores salarios.

```{r}
# Función que obtiene los coeficientes de modelo
...
p
# Guardar 
ggsave("GRÁFICO 4-DISTRIBUCIÓN DE MUESTRA.png", plot = p, dpi = 300, width = 8, height = 6)


```

En conclusion, es de notar que esta variable no es la unica que determina el valor, podriamos incluir, por ejemplo, la industria, la experiencia de laboral, la cantidad de años de estudio, ranking institucion educativa, etc. así como otras habilidades sociales que pued en ser dificiles de cuantificar, en cuyo casos podria ayudar a explicar el comportamiento del salario en función de la edad.
...
donde Female es un indicador que toma 1 si el individuo en la muestra se identifica como mujer y 0 si se identifica como hombre.
...
t_reg_female <- stargazer(reg_female, 
                          type = "text", 
                          title = "TABLA 2-MODELO DE SALARIO SEGÚN GÉNERO", 
                          dep.var.labels = "Log(Salario por Hora)", 
                          digits = 3, 
                          align = TRUE, 
                          star.cutoffs = c(0.1, 0.05, 0.01))
...
# Guardar tabla
writeLines(t_reg_female, "TABLA 2-MODELO DE SALARIO SEGÚN GÉNERO.txt")
```
...
-   El *error estándar* para este modelo es de 0.787 y esto indica la dispersión entre las 16542 observaciones y la estimación.

-   *Significancia parcial:* La variable female es significativa al 0.01 (99%), el error estándar es de 0.012 y explica que si es mujer, en promedio el salario disminuye en 7.9%

### ¿Salario igual para trabajos iguales?

El *slogan* "salario igual por trabajo igual" es una forma de interpretar que los empleados con características laborales similares, no debe existir ninguna brecha salarial de género. En esta sección, se utilizará el modelo *Frisch-Waugh-Lovell* (FWL) para estimar la brecha salarial condicional, incorporando variables de control que reflejan las características semejantes de los trabajadores y los puestos de trabajo. El procedimiento se divide en tres pasos, que se describen a continuación:

**Paso 1:** Estimación de la regresión con todos los predictores completos, y cálculo de los residuos *u*.

**Paso 2**: Se realiza una regresión auxiliar utilizando los residuos (*u*) obtenidos en el paso anterior.

**Paso 3:** Aplicar el Teorema de *Frisch-Waugh-Lovell* (FWL), donde establece que los residuos de la primera y segunda regresión son numéricamente equivalentes.
...
t_modelos3 <- stargazer(reg_female_controles, reg_residuos_female,
                        type = "text", 
                        title = "TABLA 3-COMPARACIÓN DE ESTIMACIONES DE GÉNERO EN SALARIO",
...
# Guardar tabla
writeLines(t_modelos3, "TABLA 3-COMPARACIÓN DE ESTIMACIONES DE GÉNERO EN SALARIO.txt")
```
...
coef_female <- data.frame(Beta_female = c(beta_female_controles, 
                                          beta_femal_residuos))

t_betas_female4 <- stargazer(coef_female, type = "text", 
                             summary = FALSE, 
                             title = "TABLA 4-COMPARACIÓN BETAS FEMALE")

# Guardar tabla
writeLines(t_betas_female4, "TABLA 4-COMPARACIÓN BETAS FEMALE.txt")
```
...
# Data frame para crear tabla
result_boot_errores <- data.frame(Estadístico = c("Estimación Original",
                                                  "Sesgo", "Error Estándar"
                                                  ), 
                                  Valor = c(errorestandar_boot$t0, 
                                            mean(errorestandar_boot$t) -
                                              errorestandar_boot$t0, 
                                            sd(errorestandar_boot$t)))
# Creación de tabla
t_errorstd_boot5 <- stargazer(result_boot_errores, type = "text", 
                             summary = FALSE, 
                             title = "TABLA 5-BOOTSTRAP ORDINARIO NO PARAMÉTRICO")

# Guardar tabla
writeLines(t_errorstd_boot5, 
           "TABLA 5-BOOTSTRAP ORDINARIO NO PARAMÉTRICO.txt")
```

#### Análisis de comparación de las estimaciones y los errores estándar

Al comparar las estimaciones de los coeficientes para female en los dos modelos (uno con controles y otro con los residuos de female), se observa que las estimaciones son iguales cuando se utiliza el Bootstrap.

El valor del error estándar de la estimación de female mediante el método FWL con bootstrap es 0.011, lo que es consistente con los resultados obtenidos en el modelo con controles. Además, se observa que la variabilidad en la estimación de female es más baja en el modelo con bootstrap, en comparación con el modelo FWL ajustado por los residuos, lo que sugiere una mayor precisión en las estimaciones obtenidas mediante el bootstrap.
...
```{r}
# Modelo con interacción entre edad y género
modelo_age_wage <- lm(log(salario_hora) ~ age + I(age^2) + female + age:female + I(age^2):female, 
                      data = df_salario)

# Definir edades únicas para predicción
edades <- seq(min(df_salario$age), max(df_salario$age), length.out = 100)

# Crear data frames de predicción 
df_pred_male <- data.frame(age = edades, female = 0)  # Male = 0
df_pred_female <- data.frame(age = edades, female = 1)  # Female = 1

# Predicciones separadas con intervalos de confianza
pred_male <- predict(modelo_age_wage, newdata = df_pred_male, interval = "confidence", level = 0.95)
pred_female <- predict(modelo_age_wage, newdata = df_pred_female, interval = "confidence", level = 0.95)

# Convertir predicciones de log-salario a salario real
df_pred_male$salario <- exp(pred_male[, "fit"])
df_pred_male$lower_ci <- exp(pred_male[, "lwr"])
df_pred_male$upper_ci <- exp(pred_male[, "upr"])
df_pred_male$genero <- "Male"

df_pred_female$salario <- exp(pred_female[, "fit"])
df_pred_female$lower_ci <- exp(pred_female[, "lwr"])
df_pred_female$upper_ci <- exp(pred_female[, "upr"])
df_pred_female$genero <- "Female"

# Unir los datos
df_pred <- rbind(df_pred_male, df_pred_female)

# Convertir género a factor para gráfica
df_pred$genero <- factor(df_pred$genero, levels = c("Male", "Female"))

# Gráfico con los perfiles de salario predichos
p_5 <- ggplot(df_pred, aes(x = age, y = salario, color = genero, fill = genero)) + 
  # Intervalo de confianza diferenciado por género
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), alpha = 0.3, color = NA) +
  geom_line(size = 1) + 
  scale_color_manual(values = c("Female" = "black", "Male" = "darkcyan")) +
  scale_fill_manual(values = c("Female" = "gray", "Male" = "lightblue")) +
  labs(
    title = "GRÁFICO 5-PERFIL DE EDAD SALARIO PREDICHO POR GÉNERO",
    x = "Edad",
    y = "Salario por hora")

# Guardar 
ggsave("GRÁFICO 5-PERFIL DE EDAD SALARIO PREDICHO POR GÉNERO.png", plot = p_5, dpi = 300, width = 8, height = 6)


```

Discusión de edades pico a partir del gráfico 6
...
# Guardar 
ggsave("GRÁFICO 6-DISTRIBUCIÓN DE LA DIVISIÓN DE LA MUESTRA.png", dpi = 300)
```

### Comparación del rendimiento predictivo en términos de RMSE

Para realizar una comparación significativa de los rendimientos predictivos de los modelos anteriores, también se proporcionaron cinco modelos adicionales que permiten identificar el mejor rendimiento incluyendo modelos no lienales y con mayor complejidad respecto a los estimados previamente. De este modo, se estudiaran un total 8 especificaciones:
...
**Modelo 7: Modelo no lineal con variables adicional e interacciones**

$$log(w) = β_0 + β_1Female + β_2educ + β_3tiempoempresa + β_4age + β_5formal + β_6segusocial + β_7female*segusocial + β_8age^2 + u \quad (15)$$

**Modelo 8: Modelo no lineal con variables adcionales**
...
Luego de definir los modelos, se procede a realizar las predicciones, comparacion de los resultados del RMSE e identificacion del modelo con el mejor rendimiento predictivo.
...
mse_table6 <- data.frame(
  Modelo = c("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8"),
  MSE = c(salario_1, salario_2, salario_3, salario_4, salario_5, salario_6, salario_7, salario_8)
)

# Formato tabla
tabla_6 <- stargazer(mse_table6, type = "text", summary = FALSE, title = "TABLA 6-COMPARACIÓN DE RMSE ENTRE MODELOS")

# Guardar tabla
writeLines(tabla_6, 
           "TABLA 6-COMPARACIÓN DE RMSE ENTRE MODELOS.txt")
```

### Discusión de los resultados de RMSE con Validation Set Approach
...
El Modelo 7 tiene el mejor desempeño predictivo con un RMSE de 10,940.350. Esto es consistente con su mayor nivel de complejidad, ya que incorpora no solo los controles básicos, sino también términos adicionales como la variable *formal*, *segu_social*, la interacción *female:segu_socia*l y un término cuadrático $age^2$. El Modelo 1 sigue siendo un modelo competitivo, obteniendo el segundo mejor RMSE (11,121.560). Aunque es más simple que el Modelo 7, sigue capturando buena parte de la variabilidad del salario en función de *female*, *max_nivel_educ*, *tiempo_empresa* y *age*.

Los modelos con no linealidades e interacciones presentan mejoras en algunos casos, como el Modelo 8, que incorpora la interacción *female:max_nivel_educ* y logra un RMSE similar al Modelo 1 (11,121.560). El Modelo 2 presenta el desempeño más bajo en términos predictivos (RMSE = 13,941.480). Esto es esperado, ya que este modelo solo estima los residuos de *female*, sin utilizar directamente la variable dependiente *log(salario_hora)*.
...
df_errores <- data.frame(error = errores)

h_7 <- ggplot(df_errores, aes(x = error)) +
  geom_histogram(fill = "gray70", color = "white", bins = 30) +
  labs(title = "GRÁFICO 7-DISTRIBUCIÓN DE ERRORES DE PREDICCIÓN",
       x = "Error de Predicción",
       y = "Frecuencia") +
  theme_classic() +
  theme(plot.background = element_rect(fill = "white", color = NA))  # Fondo blanco

ggsave("GRÁFICO_7_DISTRIBUCIÓN_ERRORES_PREDICCIÓN.png", plot = h_7, dpi = 300, width = 8, height = 6)
```
...
# Definir percentiles
percentiles <- quantile(df_errores$errores, probs = c(0.25, 0.75))  # Q1 y Q3

# Crear el boxplot con percentiles
box_8 <- ggplot(df_errores, aes(y = errores)) +
  geom_boxplot(fill = "gray70", color = "black") +
  geom_hline(yintercept = percentiles, color = "black", linetype = "dashed") +  # Agregar líneas de percentiles
  labs(title = "GRÁFICO 8 - BOXPLOT DE ERRORES DE PREDICCIÓN", 
...
# Guardar 
ggsave("GRÁFICO 8-BOXPLOT DE ERRORES DE PREDICCIÓN.png", plot = box_8, dpi = 300)
...
box_8 <- boxplot(df_errores, 
        main = "GRÁFICO 8 - BOXPLOT DE ERRORES DE PREDICCIÓN", 
...
A continuación, se puede visualizar una tabla comparativa de RMSE con las diferentes metolodologías aplicadas.
...
comparacion_rmse <- data.frame(
  Modelo = c("M1", "M7"),
  RMSE_VSA = c(
    sqrt(mean((exp(pred_1) - test_sample$salario_hora)^2)), 
    sqrt(mean((exp(pred_7) - test_sample$salario_hora)^2))
  ),
...
# Formato para tabla
t_rmse7 <- stargazer(comparacion_rmse, type = "text",
                   summary = FALSE,
                   title = "TABLA 7-COMPARACIÓN DE RMSE ENTRE MODELO 1 Y MODELO 7")

# Guardar tabla
writeLines(t_rmse7, 
           "TABLA 7-COMPARACIÓN DE RMSE ENTRE MODELO 1 Y MODELO 7.txt")
```
...
10. Castillo-Robayo, Cristian Darío, & García-Estévez, Javier. (2019). Desempleo juvenil en Colombia: ¿la educación importa?. Revista Finanzas y Política Económica, 11(1), 101-127. Epub October 10, 2020.https://doi.org/10.14718/revfinanzpolitecon.2019.11.1.7
