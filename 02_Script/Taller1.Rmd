...
El resultado de la estimación indica que la variable $$Age$$ genera una condicion de decrecimiento sobre el $$log(w)$$. De esta manera, tomaremos los multiplos de 18 años para realizar el analisis correspondiente. Veamos, cuando la variable $$Age$$ es igual a 18 años,  $$log(w)$$ es igual a XXXX. Ahora, cambiamos el valor a 36 años, $$log(w)$$ es igual a XXXX. por último, $$Age$$ sera 54 años y por lo tanto $$log(w)$$ es igual a XXXX. 
...
Con el analisis anterior, podemos observar el comportamiento de la ecuación estimada es parabolico, es decir, en la "edad pico" es el punto de inflexión. Es decir, todas las edades menores a la "edad pico", la variable $$log(w)$$ será cerciente y para edades mayores a la "edad pico" la varible $$log(w)$$ seran decreciente. Asi las cosas, es necesario conocer como se calcula el punto de inflexión.
...
Calculo de RMSE por valor real
...
# Generar predicciones y RMSE
pred_1_log <- predict(m1, test_sample)
salario_1_log <- RMSE(pred_1_log, log(test_sample$salario_hora))

pred_2_log <- predict(m2, test_sample)
salario_2_log <- RMSE(pred_2_log, log(test_sample$salario_hora))

pred_3_log <- predict(m3, test_sample)
salario_3_log <- RMSE(pred_3_log, log(test_sample$salario_hora))

pred_4_log <- predict(m4, test_sample)
salario_4_log <- RMSE(pred_4_log, log(test_sample$salario_hora))

pred_5_log <- predict(m5, test_sample)
salario_5_log <- RMSE(pred_5_log, log(test_sample$salario_hora))

pred_6_log <- predict(m6, test_sample)
salario_6_log <- RMSE(pred_6_log, log(test_sample$salario_hora))

pred_7_log <- predict(m7, test_sample)
salario_7_log <- RMSE(pred_7_log, log(test_sample$salario_hora))

pred_8_log<- predict(m8, test_sample)
salario_8_log <- RMSE(pred_8_log, log(test_sample$salario_hora))

# Crear la tabla con los MSE
mse_table <- data.frame(
  Modelo = c("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8"),
  MSE = c(salario_1_log, salario_2_log, salario_3_log, salario_4_log, salario_5_log, salario_6_log, salario_7_log, salario_8_log)
)
...
errores <- test_sample$salario_hora - predict(m7, test_sample)
...
rmse_m1_loocv_log <- RMSE(m1_loocv$pred$pred, m1_loocv$pred$obs)

cat("Modelo 1 - LOOCV RMSE (Log):", round(rmse_m1_loocv_log, 6), "\n")
...
# Calcular RMSE en log
rmse_m7_loocv_log <- RMSE(m7_loocv$pred$pred, m7_loocv$pred$obs)

cat("Modelo 7 - LOOCV RMSE (Log):", round(rmse_m7_loocv_log, 6), "\n")

```

LOOCV m1 con valor real
...
comparacion_rmse <- data.frame(
  Método = c("Validation Set", "Validation Set", "LOOCV", "LOOCV"),
  Modelo = c("M1", "M7", "M1", "M7"),
  RMSE_Log = format(c(salario_1_log, salario_7_log, rmse_m1_loocv_log, rmse_m7_loocv_log), scientific = FALSE),
  RMSE_Original = c(sqrt(mean((exp(pred_1_log) - test_sample$salario_hora)^2)), 
...
LOOCV con laverage m1

```{r}
m1_full <- lm(form_1, data = df_salario)

X<- model.matrix(m1_full)
y <- model.response(model.frame(m1_full))

beta_hat <- m1_full$coefficients
  
## Calculate the inverse of  (X'X), call it G_inv
G_inv<- solve(t(X)%*%X)

## and 1/1-hi
vec<- 1/(1-hatvalues(m1_full))
...
for (i in 1:N) {
  # get the new beta
  new_beta<- beta_hat  - vec[i] * G_inv %*% as.vector(X[i, ]) * m1_full$residuals[i]
    ## get the new error
...
LOOCV con laverage m7

```{r}
m7_full <- lm(form_7, data = df_salario)

X<- model.matrix(m7_full)
y <- model.response(model.frame(m7_full))

beta_hat <- m7_full$coefficients
  
## Calculate the inverse of  (X'X), call it G_inv
G_inv<- solve(t(X)%*%X)

## and 1/1-hi
vec<- 1/(1-hatvalues(m7_full))
...
for (i in 1:N) {
  # get the new beta
  new_beta<- beta_hat  - vec[i] * G_inv %*% as.vector(X[i, ]) * m7_full$residuals[i]
    ## get the new error
    new_error<- (y[i]- (X[i, ] %*% new_beta))^2
    LOO[i]<-  new_error
  }
