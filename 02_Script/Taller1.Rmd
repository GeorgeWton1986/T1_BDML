...
# Realizar el remuestreo (bootstrap) de tamaño 500 con reemplazo
boot <- sample(df_salario$age,500, replace = TRUE)

#Crear una matriz de 500 filas y 1 columna a partir del remuestreo
m_boots <- matrix(boot, nrow = 500, ncol = 1, byrow = TRUE)

#Calcular los intervalos de confianza de la matriz
intervalos <- quantile(m_boots, probs = c(0.025,0.975))

intervalos
```

El metodo de Bootstrap permite entender la variabilidad de los datos estadisticos que puede ser usado 
para cuantificar la incertidumbre de los coeficientes calculados. Esta herramienta es usada para estimar
los errores estandar de los estimadores desde una regresion ajustada.
...
```{r}
#Se comparan los coeficienties de female
